...
def get_evil_word(url):
    return len(re.findall("(alert)|(script)|(onerror)|(onclick)|(onload)|(eval)|(src)|(prompt)|(iframe)|(style)",url,re.IGNORECASE))

def get_feature(url):
    return [get_evil_char(url),get_transform_char(url),get_evil_word(url),get_url_count(url),get_len(url)]
...
clf = svm.SVC(kernel='rbfr', C=1).fit(x_train, y_train)
